...
export class CalendarComponent extends React.Component<Props> {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < this.listNameOfDay.length; j++) {
        ...
        if (
          ...
          this.listJobOfWorker
        ) {
          jobs = this.listJobOfWorker.filter(
            (e) => e.jobdate.getTime() === cell.getTime()
          );
        }
        if (this.listWorkerOfJob) {
          workers = this.listWorkerOfJob.filter(
            (e) => e.jobdate.getTime() === cell.getTime()
          );
        }
        calendars.push({
  ...
  calendarCellJob() {
    return this.calendarRows.map((row, index) => (
