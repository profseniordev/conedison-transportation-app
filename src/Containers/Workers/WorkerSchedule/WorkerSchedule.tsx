...
export class WorkerSchedule extends React.Component<any> {
  ...
  getJobsByDate = (date) => {
    const jobs: FiltredJob[] = this.props.jobs.reduce((jobs, job) => {
      const workers = job.workers.filter((worker) => {
        ...
        return (
          moment(worker.startDate).isAfter(moment(date, 'YYYYMMDD'), 'D') &&
  ...
  renderCellInfo(date, item?: any) {
    ...
    if (jobs.length === 1) {
      ...
      return (
        <div
          className="btn-calendar-worker-wrapper"
          onClick={() => this.onAddRaised(item)}
        >
          <span
            className="btn cursor-pointer btn-calendar-worker sbtw"
            onClick={() => {}}
          >
            <span>{JobType[job.job.jobType]}</span>
            <span> {job.job.uid}</span>
          </span>
          <span
            className="btn cursor-pointer btn-calendar-worker btn-calendar-worker-red sbtw"
            style={{ marginTop: 10 }}
            onClick={() => {}}
          >
            <span>{formatDate(worker.startDate, FORMATES.time)}</span>
            {worker.endDate ? (
              <span> - {formatDate(worker.endDate, FORMATES.time)}</span>
            ) : null}
          </span>
        </div>
      );
    ...
    if (jobs.length) {
      return (
        <span
          className="btn btn-calendar-worker cursor-pointer"
          onClick={() => this.onAddRaised(item)}
        >
